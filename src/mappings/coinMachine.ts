...
import {
  ...
  TokensBought,
  TokensBought1 as HistoricTokensBought,
  PeriodUpdated,
...
export function handleHistoricTokensBought(event: HistoricTokensBought): void {
  let extension = CoinMachineContract.bind(event.address);
  let colony = extension.getColony();
...
export function handleHTokensBought(event: TokensBought): void {
  let extension = CoinMachineContract.bind(event.address);
  let colony = extension.getColony();
  ...
  handleEvent("TokensBought(address,address,uint256,uint256)", event, colony)
}
